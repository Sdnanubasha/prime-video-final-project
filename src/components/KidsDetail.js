...
const KidsDetail = () => {
    ...
    const AddToMystuff = async () =>{
      try {
        let result = await movie.map(post =>( axios.post(`https://prime-video-api.onrender.com/mystuff-kids?id=${post.k_id}`)))
        if(result) {
    ...
    const getMovie = async () => {
        try {
            let res = await axios.get(`https://prime-video-api.onrender.com/amazon-kids?id=${movieID}`);
                setMovie(res.data)
        } catch (error) {
            console.log(error);
        }
              ...
              <div className="movie-part col-md-12">
                <ul className="movie-item">
                  ...
                  <p><b>Description : </b>{post.description}</p>
                    ...
                    <Link to="/kids"><button className="btn btn-danger">Back</button></Link>
                  </div>
                </ul>
              </div>
            </div>
